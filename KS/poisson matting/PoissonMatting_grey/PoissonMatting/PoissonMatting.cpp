...
void gradient(cv::Mat src, cv::Mat& x, cv::Mat& y) {
	x.create(src.size(), CV_32FC1);
	y.create(src.size(), CV_32FC1);

	for (int i = 0; i < src.rows; i++)
	{
		for (int j = 0; j < src.cols; j++)
		{
			if (i == 0) y.at < float >(i, j) = src.at<uchar>(i + 1, j) - src.at<uchar>(i, j);
			else if (i == src.rows - 1) y.at<float>(i, j) = src.at<uchar>(i, j) - src.at<uchar>(i - 1, j);
			else y.at<float>(i, j) = (float)(src.at<uchar>(i + 1, j) - src.at<uchar>(i - 1, j)) / 2;

			if (j == 0) x.at<float>(i, j) = src.at<uchar>(i, j + 1) - src.at<uchar>(i, j);
			else if (j == src.cols - 1) x.at<float>(i, j) = src.at<uchar>(i, j) - src.at<uchar>(i, j - 1);
			else x.at<float>(i, j) = (float)(src.at<uchar>(i, j + 1) - src.at<uchar>(i, j - 1)) / 2;
		}
	}
}
...
void initDiff(cv::Mat& mask, const cv::Mat& grey, cv::Mat& output) {
	...
	//	Mat pic = Mat::zeros(mask.rows, mask.cols, CV_8U);
			...
			if (p[x] == GREY)
			{
				...
				for (auto& it : pixels)
				{
					...
					if (msk == WHITE)
					{
						...
						f_pixel = grey.at<uchar>(absoluteY, absoluteX);
						break;
					}
				}
				for (auto& it : pixels)
				{
					...
					if (msk == BLACK)
					{
						//cout << "black." << endl;
						//pic.at<uchar>(absoluteY, absoluteX) = 200;
						b_pixel = grey.at<uchar>(absoluteY, absoluteX);
						break;
					}
				}
				//cout << f_pixel << "," << b_pixel << endl;
				output.at<float>(y, x) = f_pixel / 255.f - (float)b_pixel / 255.f;
			}
		}
	}
