			...
			if (trimap.at<uchar>(y, x) == GREY) {
				...
				for (int i = 0; i < 8; i++) {
					...
					if (q.x < trimap.cols && q.x >= 0 && q.y < trimap.rows&& q.y >= 0) {
						...
						Vec3f F_gradient = offsets[i].x*Ix.at<Vec3f>(NB_pixel.F_point) + offsets[i].y*Iy.at<Vec3f>(NB_pixel.F_point);
						Vec3f B_gradient = offsets[i].x*Ix.at<Vec3f>(NB_pixel.B_point) + offsets[i].y*Iy.at<Vec3f>(NB_pixel.B_point);
						Vec3f D = alpha0.at<float>(y, x)*F_gradient + (1 - alpha0.at<float>(y, x))*B_gradient;

						NB_pixel.sum_v = NB_pixel.sum_v - dot(I-D, dif) / dif_value;
					}
				}
